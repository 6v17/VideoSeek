...
from src.storage.lance_store import build_video_lookup
...
def list_dialogue_index_targets(*, config=None) -> list[dict[str, str]]:
    """Ready library videos that exist on disk (candidates for subtitle indexing)."""
    from src.storage.asset_store import load_metadata

    cfg = config
    model_dirs = get_local_model_asset_dirs(config=cfg)
    meta_path = model_dirs["meta_file"]
    ...
    for root_path, lib_data in libraries.items():
        ...
        if not isinstance(files, dict):
            continue
        for _rel, info in files.items():
            if not isinstance(info, dict):
                continue
            video_id = str(info.get("vid", "") or "").strip()
            if not video_id:
                continue
            state = str(info.get("asset_state", "") or "").strip().lower()
            if state and state not in {"ready", ""}:
                continue
            entry = lookup.get(video_id) or {}
            ...
            targets.append(
                {
                    ...
                    "library_path": str(entry.get("library_path", "") or root_path),
                }
            )
    return targets
