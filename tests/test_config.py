    ...
    def test_load_config_rewrites_legacy_install_paths_without_copying_data_dir(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with (
                ...
                patch.object(config_module, "LEGACY_INSTALL_DIR", str(legacy_root)),
                ...
                self.assertFalse((user_data_dir / "meta.json").exists())

    def test_new_user_empty_legacy_data_dir_gets_user_paths_immediately(self):
        """Bundled empty install-dir data/ must not block startup or defer path migration."""
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            legacy_root = root / "legacy_app"
            legacy_root.mkdir()
